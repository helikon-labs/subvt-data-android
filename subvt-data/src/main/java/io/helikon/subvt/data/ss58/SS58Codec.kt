...
internal fun ByteArray.copyBytes(
    from: Int,
    size: Int,
) = copyOfRange(from, from + size)
...
internal object SS58Codec {
    ...
    private fun getPrefixLenIdent(decodedByteArray: ByteArray): Pair<Int, Short> {
        return when {
            ...
            decodedByteArray[0] in 64..16383 -> {
                val lower =
                    (
                        ((decodedByteArray[0] and 0b00111111).toInt() shl 2) or (
                            decodedByteArray[1].toUByte()
                                .toInt() shr 6
                        )
                    ).toShort()
    ...
    private fun encode(
        ...
        val addressTypeByteArray =
            when (val ident = addressByte and 0b00111111_11111111) {
    ...
    private fun extractAddressByteOrNull(address: String): Short? =
        try {
